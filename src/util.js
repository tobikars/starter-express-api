...
module.exports = function (req, body, url, res) {
  ...
  if (N < 0)
    switch (url.query.t || 'csv') {
      case 'json':
        return res.status(200).send(JSON.stringify(wb.SheetNames))
      default:
        return res.status(200).send(wb.SheetNames.join('\n'))
    }
  ...
  switch (url.query.t) {
    case 'json':
      return res.status(200).json(XLSX.utils.sheet_to_json(ws, { header: 1, raw: false }))
    case 'html':
      return XLSX.stream.to_html(ws).pipe(res)
    default:
      return XLSX.stream.to_csv(ws).pipe(res)
  }
}
